"""
profile_lubrication_cupy.py
----------------------------
Drop-in replacement for lubrication_cupy.py that times each major section
of ResistCSC_both using cp.cuda.Event for accurate GPU-side measurements.

Accumulated timings are stored in Lubrication.profile and printed via
Lubrication.print_profile(). Reset with Lubrication.reset_profile().

Usage:
    from profile_lubrication_cupy import Lubrication as LubricationCuPy
    lc = LubricationCuPy(d_cut)
    # ... run as normal ...
    lc.print_profile()
"""
import numpy as np
...
from lubrication_cupy import (
    _DEFAULT_DTYPE, _get_coeffs, _6PI,
    _RPY_HSCALE, _DR_HSCALE,
    _RPY_PMIN, _RPY_NP, _RPY_NQ,
    _DR_PMIN,  _DR_NP,  _DR_NQ,
    _G_EMROW, _G_EMCOL,
    _ENTRY_BLOCK, _ENTRY_LROW, _ENTRY_LCOL,
    _eval_rat_wall_all, _rpy_wall_batch, _asym_wall_batch,
    _delta_R_wall_batch, _wall_matrices_batch,
    _eval_pair_PQ_batch, _pair_scalars_batch, _assemble_pair_triplets,
)
...
class _GpuTimer:
    """Accumulates GPU-side elapsed time using cp.cuda.Event pairs."""
    def __init__(self):
        self.ms   = 0.0
        self.hits = 0

    def add(self, start_event, end_event):
        end_event.synchronize()
        self.ms   += cp.cuda.get_elapsed_time(start_event, end_event)
        self.hits += 1
...
def _evt():
    """Create and record a new cuda Event."""
    e = cp.cuda.Event()
    e.record()
    return e
...
# =============================================================================
# Public Lubrication class (profiled)
# =============================================================================

class Lubrication:
    """
    Profiled CuPy lubrication class. Same interface as lubrication_cupy.Lubrication.
    After one or more ResistCSC_both calls, inspect timings with print_profile().
    """
    ...
    def reset_profile(self):
        self.profile = {k: _GpuTimer() for k in self._SECTIONS}

    def print_profile(self):
        calls = self.profile['total'].hits
        print(f"\n{'─'*58}")
        print(f"  CuPy lubrication profile  ({calls} call{'s' if calls!=1 else ''})")
        print(f"{'─'*58}")
        print(f"  {'section':<28} {'total ms':>10} {'per call':>10} {'%':>6}")
        print(f"  {'─'*54}")
        total_ms = self.profile['total'].ms
        for k in self._SECTIONS:
            t = self.profile[k]
            pct = 100.0 * t.ms / total_ms if total_ms > 0 else 0.0
            per = t.ms / calls if calls > 0 else 0.0
            print(f"  {k:<28} {t.ms:>10.2f} {per:>10.3f} {pct:>6.1f}%")
        print(f"{'─'*58}\n")

    # Physical cutoff for pair rational fits — hardcoded (fits only valid to 4.5a)
    _PAIR_CUTOFF = 4.5
    ...
    def ResistCSC(self, r_vectors, n_list, a, eta, cutoff,
        ...
        R_MB, R_Sup = self.ResistCSC_both(
            r_vectors, n_list, a, eta, periodic_length=periodic_length)
        return R_Sup if Sup_if_true else R_MB

    def ResistCOO_both(self, r_vectors, n_list, a, eta,
                       cutoff=None, wall_cutoff=None, periodic_length=None,
                       r_gpu=None, j_gpu=None, k_gpu=None, pl_gpu=None):
        """Profiled version of ResistCOO_both — returns (COO_mb, COO_sup) on GPU.
        cutoff and wall_cutoff are ignored; pair cutoff is hardcoded to 4.5a
        and wall corrections are always applied to all particles.
        """
        pr   = self.profile
        t0   = _evt()
        ...
        if r_gpu is not None:
            ...
            r_g = cp.asarray(
                ...
                dtype=dtyp)
        t1 = _evt(); pr['positions_and_pl'].add(t0, t1)

        rows_mb, cols_mb, vals_mb = [], [], []
        rows_sup, cols_sup, vals_sup = [], [], []

        # ── Wall: always applied to ALL N particles ───────────────────────
        # (wall_mask section now just measures the eps clamping operation)
        h_all   = r_g[:, 2]
        eps_all = cp.maximum(h_all - 1.0, cp.dtype(dtyp).type(self.debye_cut))
        h_all   = 1.0 + eps_all
        all_idx = cp.arange(N, dtype=cp.int32)
        t2 = _evt(); pr['wall_mask'].add(t1, t2)

        # ── Wall: RPY + Delta_R (always in float64 to avoid f32 cancellation) ─
        C64   = _get_coeffs(cp.float64)
        h64   = h_all.astype(cp.float64)
        e64   = eps_all.astype(cp.float64)
        rpy   = _rpy_wall_batch(h64, C64).astype(dtyp)
        dR    = _delta_R_wall_batch(e64, h64,
                                    rpy.astype(cp.float64), C64).astype(dtyp)
        t3    = _evt(); pr['wall_rpy_and_dR'].add(t2, t3)

        # ── Wall: fill 6×6 blocks ─────────────────────────────────────────
        def fill_6x6(Xa, Ya, Yb, Xc, Yc):
            R = cp.zeros((N, 6, 6), dtype=dtyp)
            R[:, 0, 0] = f0*Ya;  R[:, 1, 1] = f0*Ya;  R[:, 2, 2] = f0*Xa
            R[:, 3, 3] = f2*Yc;  R[:, 4, 4] = f2*Yc;  R[:, 5, 5] = f2*Xc
            R[:, 0, 4] =  f1*Yb; R[:, 4, 0] =  f1*Yb
            R[:, 1, 3] = -f1*Yb; R[:, 3, 1] = -f1*Yb
            return R
        R_rpy_blk = fill_6x6(rpy[0], rpy[1], rpy[2], rpy[3], rpy[4])
        R_sup_blk = fill_6x6(rpy[0]+dR[0], rpy[1]+dR[1], rpy[2]+dR[2],
                             rpy[3]+dR[3], rpy[4]+dR[4])
        t4 = _evt(); pr['wall_fill_6x6'].add(t3, t4)
        ...
        def wall_triplets(R_blk):
            ...
            mask = cp.abs(gv) > thr
            if mask.any():
                return gr[mask], gc[mask], gv[mask]
            return None, None, None

        gr, gc, gv = wall_triplets(R_rpy_blk)
        if gr is not None:
            rows_mb.append(gr); cols_mb.append(gc); vals_mb.append(gv)
        t5 = _evt(); pr['wall_triplets_mb'].add(t4, t5)

        gr, gc, gv = wall_triplets(R_sup_blk)
        if gr is not None:
            rows_sup.append(gr); cols_sup.append(gc); vals_sup.append(gv)
        t6 = _evt(); pr['wall_triplets_sup'].add(t5, t6)
        ...
        t7 = _evt(); pr['pair_index_build'].add(t6, t7)
        ...
        if has_pairs:
            ...
            t8 = _evt(); pr['pair_displacements'].add(t7, t8)

            r_norms_g = cp.sqrt((d_g * d_g).sum(axis=1))
            in_cut    = r_norms_g < self._PAIR_CUTOFF   # hardcoded 4.5a
            t9 = _evt(); pr['pair_norms_and_cut'].add(t8, t9)
            ...
            if in_cut.any():
                ...
                sc_mb = _pair_scalars_batch(r_norms_g, self.debye_cut,
                                            C['_CF_MB'],  False)
                t10 = _evt(); pr['pair_scalars_mb'].add(t9, t10)

                sc_su = _pair_scalars_batch(r_norms_g, self.debye_cut,
                                            C['_CF_SUP'], True)
                t11 = _evt(); pr['pair_scalars_sup'].add(t10, t11)

                pr_mb, pc_mb, pv_mb = _assemble_pair_triplets(
                    sc_mb, j_g, k_g, n_dof, f0, f1, f2, r_hat_g)
                if pr_mb.size > 0:
                    rows_mb.append(pr_mb); cols_mb.append(pc_mb); vals_mb.append(pv_mb)
                t12 = _evt(); pr['pair_assembly_mb'].add(t11, t12)

                pr_su, pc_su, pv_su = _assemble_pair_triplets(
                    sc_su, j_g, k_g, n_dof, f0, f1, f2, r_hat_g)
                if pr_su.size > 0:
                    rows_sup.append(pr_su); cols_sup.append(pc_su); vals_sup.append(pv_su)
                t13 = _evt(); pr['pair_assembly_sup'].add(t12, t13)
            else:
                t13 = t9
        else:
            t13 = t7
        ...
        R_MB_coo = build_coo(rows_mb, cols_mb, vals_mb)
        t14 = _evt(); pr['build_coo_mb'].add(t13, t14)

        R_Sup_coo = build_coo(rows_sup, cols_sup, vals_sup)
        t15 = _evt(); pr['build_coo_sup'].add(t14, t15)

        return R_MB_coo, R_Sup_coo

    def ResistCSC_both(self, r_vectors, n_list, a, eta,
                       cutoff=None, wall_cutoff=None, periodic_length=None,
                       r_gpu=None, j_gpu=None, k_gpu=None, pl_gpu=None):
        """cutoff and wall_cutoff accepted for API compatibility but ignored."""
        pr  = self.profile
        t0  = _evt()
        R_MB_coo, R_Sup_coo = self.ResistCOO_both(
            r_vectors, n_list, a, eta, periodic_length=periodic_length,
            r_gpu=r_gpu, j_gpu=j_gpu, k_gpu=k_gpu, pl_gpu=pl_gpu)

        t_a = _evt()
        R_MB = R_MB_coo.tocsc()
        t_b  = _evt(); pr['tocsc_mb'].add(t_a, t_b)

        R_Sup = R_Sup_coo.tocsc()
        t_c   = _evt(); pr['tocsc_sup'].add(t_b, t_c)
