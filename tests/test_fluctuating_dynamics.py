...
dt            = 0.25           # timestep
t_max         = 1.1e3            # total simulation time
n_steps       = int(t_max / dt)
n_save        = 1            # save radial coordinate every n_save steps

# Periodic box (x, y periodic; z open above wall)
Lx, Ly        = 160.0, 160.0
L             = np.array([Lx, Ly, 0.0])
z_max_solver  = 2.0 * (2.0 * a)

# Area fraction and number of particles
phi           = 0.66
...
# Output file prefix (used for all npz and png files)
out_dir    = os.path.dirname(os.path.abspath(__file__))
out_prefix = os.path.join(out_dir,
                          f'msd_phi{phi:.2f}_L{Lx:.0f}x{Ly:.0f}')
...
# First row = packing fractions; first column = time (s).
# Each data value = 2 * MSD_parallel, so MSD = value / 2.
# Data from A. Thorneywork, E. Mackay - https://doi.org/10.1103/PhysRevX.14.041016
# =============================================================================
...
# MSD = 2*MSD_data / 2 = _EXP_2MSD / 2
_EXP_MSD = _EXP_2MSD / 2.0

def _get_exp_msd(phi_sim, tol=0.005):
    """
    Return (t, msd) arrays for the experimental packing fraction closest to
    phi_sim, or (None, None) if no experimental phi is within tol.
    """
...
# =============================================================================
# MSD plot/save helper
# =============================================================================
def plot_and_save_msd(msd_mean, msd_err, lag_times, tag, t_sim=None):
    """
    Save npz and png for the given MSD arrays.
    tag    : string appended to out_prefix (e.g. 'frame000050' or 'final')
    t_sim  : simulation time for plot title; None = final run
    Overlays experimental MSD if available for the current phi.
    """
    D0_free = kT / (6.0 * np.pi * eta * a)
    D_meas  = msd_mean[1] / (2.0 * lag_times[1]) if lag_times[1] > 0 else float('nan')
    ...
    # overlay experimental data if available for this phi
    ...
    ax.plot(t_pl, msd_pl, '--', color='steelblue', lw=1.5, label='MSD sim (parallel)')
...
def msd_matrix(matrix):
    """
    Compute FFT-based MSD for each row of matrix (shape N_particles x T_frames).
...
# =============================================================================
# Initialise particles — non-overlapping, z ~ exponential wall distribution
# =============================================================================
@njit(parallel=True, fastmath=True)
def _check_overlaps(positions, x, y, z, min_sep_sq, n_placed, Lx, Ly):
    """
    Returns True if (x, y, z) overlaps with any of the first n_placed
    entries in positions.  Runs parallel over existing particles.
    x, y wrapping uses minimum-image only for the open-geometry case
    (Lx, Ly <= 0 disables wrapping).
    """
    overlaps = False
...
def place_particles(N, a, kT, g, Lx, Ly, seed=42,
    ...
    z-coordinate: particle centre = a + wall_gap,
                  wall_gap ~ Exponential(z_mean),  z_mean = kT/g
 
    If `max_failures_before_increase` consecutive placement attempts
    fail, z_mean is increased by `retry_increase` (default 10%) and
    attempts restart for the current particle.
 
    Parameters
    ----------
    N                           : number of particles
    a                           : particle radius
    kT, g                       : thermal energy, gravitational acceleration
    Lx, Ly                      : box dimensions (>0 → periodic min-image
                                  overlap check; <=0 → open)
    seed                        : RNG seed
    retry_increase              : fractional increase of z_mean per failure burst
    max_failures_before_increase: attempts before z_mean is increased;
                                  defaults to 10*N
    ...
    while n_placed < N:
        ...
        while not placed:
            ...
            if n_placed == 0:
                # first particle always accepted
                overlaps = False
            else:
                overlaps = _check_overlaps(
                    positions, x, y, z, min_sep_sq, n_placed, Lx, Ly)
 
            if not overlaps:
                positions[n_placed, 0] = x
                positions[n_placed, 1] = y
                positions[n_placed, 2] = z
                n_placed += 1
                placed    = True
            else:
                failures += 1
...
# =============================================================================
# Trajectory storage — radial coordinate r = sqrt(x^2 + y^2) per particle
# Unwrapped positions used so r grows diffusively (no modular jumps).
# =============================================================================
n_frames   = n_steps // n_save + 1
traj_r     = np.zeros((N, n_frames))   # (N_particles, T_frames)
frame_idx  = 0
save_times = np.zeros(n_frames)

r_init    = np.array([b.location for b in bodies])   # unwrapped at t=0
...
for step in range(1, n_steps + 1):
    ...
    # save r_now to an npz file every iteration and overwrite the same file (for debugging)
    np.savez('debug_positions.npz', step=step, time=step*dt, positions=r_now)
    solver.Update_Bodies_Trap(FT_calc, stochastic=True) #, print_residual=True
    #solver.print_timings()

    if step % n_save == 0:
        frame_idx += 1
        r_cur     = np.array([b.location for b in bodies])   # unwrapped
        ...
        if frame_idx % 50 == 0:
            ...
            # --- compute MSD on frames collected so far and save ---
...
# =============================================================================
# Compute MSD via FFT for each particle, then average across particles
# traj_r shape: (N_particles, T_frames)
# msd_matrix returns MSD[i, lag] for particle i at lag index m
# =============================================================================
...
plot_and_save_msd(msd_mean, msd_err, lag_times, tag='final')
